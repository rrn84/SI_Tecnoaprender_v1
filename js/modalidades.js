...
$(document).ready(function () {
  ...
  $('.close').click(function() {
    ...
    $(document).on('click', '.btn-edit', function(){
      ...
      console.log("Modalidad #: "+modalidad_id); 
      $("#modalidad_educativa").val($(this).attr("element"));
      $('#dataModal').modal('show'); 
    });
    $(document).on('click', '.btn-del', function(){
      var modalidad_id = $(this).attr("id_elemento");  
      console.log("Modalidad No.: "+modalidad_id); 
      $("#elimina-a").text("¿Está seguro que desea eliminar la modalidad "+$(this).attr("element")+"?");
      modalidadTemp = modalidad_id;
      $('#mi-modal').modal('show'); 
    });
    $("#modal-btn-si").click(function (e) { 
      e.preventDefault();
      eliminarModalidad(modalidadTemp,"modalidad_educativa");
      $('#mi-modal').modal('hide');
    });
    $("#btn-actualizar").click(function (e) { 
      e.preventDefault();
      editarModalidad(modalidadTemp,"modalidad_educativa");
      $('#dataModal').modal('hide');
    });
    $("#btn-agregar").click(function (e) { 
      var modalidad = $("#modalidadNueva").val(); 
      e.preventDefault();
      if (modalidad == "") {
        console.log("Debe ingresar un valor");
        $("#mensajeError").show();
      } else {
        agregarModalidad(modalidad,"modalidad_educativa");
        $('#agregarModal').modal('hide');
      }
     
    });
    $("#agregarModalidad").click(function (e) { 
      e.preventDefault();
      $("#modalidadNueva").val("");
...
function eliminarModalidad(element,tabla) {
...
function agregarModalidad(element,tabla) {
  console.log("Agregando a "+element+" a "+tabla);
  data= new FormData();
  //data.append("id_elemento",element);
  //data.append("tabla",tabla);
  data.append("modalidad",element);
  $.ajax({
...
const data = new FormData();
 url= '../server/consultar_modalidades.php';
 fetch( url)
         ...
         "</table>"
               ...
               for (let index = 0; index < limite; index++) {
                 ...
                 row = $(
                   ...
                   "<i element='"+array[index].modalidad+"'id='"+ array[index].id +"'class='fas fa-pencil-alt lnk-ico btn-edit'></i>" +
                 "</td>" +
                 "<td class='btn-edit-del'>" +
                     "<i element='"+array[index].modalidad+"' id_elemento='"+ array[index].id +"' id='faiDel"+ index +"'class='far fa-trash-alt lnk-ico btn-del'></i>" +
                 "</td>" +
                   "</tr>"
               );
                 $(tBody).append(row);
